...
The current runtime is the content-hashed JavaScript module referenced by the root `index.html`. It contains the H164 viewer and its defaults, with only the pendant and single ring bundled. The older `SOURCE/src` tree remains historical reference; rebuilding from it alone will not reproduce H164.
