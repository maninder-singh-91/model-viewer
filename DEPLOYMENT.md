...
- The settings panel and its toggle are available on desktop and mobile, including material selection and per-geometry normal controls.
...
- Camera > Save default view stores the current camera position, target, up vector, field of view, and zoom separately for each model. Loading the model or clicking Reset view restores it. Clear default view removes only that model's saved view.
- Saved views are local to this browser and origin, keyed by the model's SHA-256 hash. They are not uploaded to GitHub, transferred from localhost to a hosted site, or included in settings JSON. Existing zoom limits remain active.
